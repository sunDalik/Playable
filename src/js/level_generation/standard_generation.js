...
function setStartPosition(startRoom) {
    ...
    if (true) {
        ...
        level[startRoom.offsetY + startRoom.height - 3][startRoom.offsetX + startRoom.width - 3].entity = new BombSkull(startRoom.offsetX + startRoom.width - 3, startRoom.offsetY + startRoom.height - 3);
    }
}
...
function generateEnemies() {
    ...
    for (const room of rooms) {
        if ([ROOM_TYPE.MAIN, ROOM_TYPE.SECONDARY].includes(room.type)) {
            ...
            let enemyAmount = Math.round(emptyTiles / 8) + randomBonus;
            if (enemyAmount <= 0 && Math.random() < 0.85) enemyAmount = 1;
            let pack;
...
function generateLaserTurrets() {
    let laserTurretsAmount = Math.round((level[0].length * level.length) * 0.008);
    for (const point of getValidRoomPoints(new Room(0, 0, level[0].length, level.length))) {
