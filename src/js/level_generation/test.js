...
//connects room by room, finds the closest entries of all room entries
function nothing() {
    let level = [[]];
    let roomNumber;
    let levelRoomWidth;
    ...
    let connectedRooms = [];
    const maxCorridorLength = Math.min(levelTileWidth, levelTileHeight) * 0.32;
    const maxExitCount = 4;
    const maxRoomId = levelRooms.length - 1;
    const sufficientCorridorLength = 7;
    let roomArray = [];
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        for (const r of roomArray) {
            if (connectedRooms.includes(r)) continue;
            const entries = entryPoints.filter(e => e.room_id === r);
            const connections = [];
            for (const entry of entries) {
                if (!entry.connected) {
                    const minConnection = getMinimalConnection(levelGraph, roomConnections, entry, entryPoints);
                    if (minConnection !== undefined) {
                        connections.push(minConnection);
                        minConnection.start_entry = entry;
                        if (minConnection.connection.length <= sufficientCorridorLength) break;
                    }
                }
            }
            const minConnection = connections.reduce((acc, curr) => curr.connection.length < acc.connection.length ? curr : acc, connections[0]);
            if (minConnection !== undefined) {
                if (minConnection.connection.length > maxCorridorLength && i > 1) continue;
                if (entryPoints.reduce((acc, curr) => curr.room_id === r && curr.connected ? acc + 1 : acc, 0) > maxExitCount) continue;
                connectEntries(minConnection.entry, minConnection.start_entry, minConnection.connection, roomConnections, level);
                connectedRooms.push(minConnection.entry.room_id);
            }
        }
        randomShuffle(roomArray);
    }
    ...
    //ensure that we can reach any room from any other room
    for (let r = 0; r <= maxRoomId; r++) {
        const startEntry = entryPoints.find(e => e.room_id === r && e.connected);
        const unreachableRooms = [];
        for (let r2 = 0; r2 <= maxRoomId; r2++) {
            if (r === r2) continue;
            const endEntry = entryPoints.find(e => e.room_id === r2);
            const result = Game.finder.findPath(startEntry.coords.x, startEntry.coords.y, endEntry.coords.x, endEntry.coords.y, levelPlayerGraph.clone());
            if (result.length === 0) {
                unreachableRooms.push(r2);
            }
        }
        for (let ur = unreachableRooms.length - 1; ur >= 0; ur--) {
            const endEntries = entryPoints.filter(e => e.room_id === ur);
            const startEntries = entryPoints.filter(e => e.room_id === r);
            const connections = [];
            for (const entry of startEntries) {
                const minConnection = getMinimalConnection(levelGraph, roomConnections, entry, endEntries, false);
                if (minConnection !== undefined) {
                    minConnection.start_entry = entry;
                    connections.push(minConnection);
                    if (minConnection.connection.length <= sufficientCorridorLength) break;
